...
public:
  ...
  std::vector<Eigen::Vector3f> &frame_buffer_resovled() {
    return frame_buf_resolved;
  }
...
private:
  ...
  std::vector<Eigen::Vector3f> frame_buf_resolved;

  int super_sampler_1d = 2;
  std::vector<float> depth_buf;
  int get_index(int x, int y);
  int get_super_sampled_index(int x, int y, int k);
